    ...
    try {
        ...
        if (messageFile) {
            ...
            if (!uploadedmessageFile) {
                return NextResponse.json({ message: "Error uploading messageFile" }, { status: 500 });
            }
        }
...
export async function GET(req: NextRequest, { params }: { params: { receiverId: string } }) {
    try {
        ...
        if (!user) {
            return NextResponse.
                json({
                    message: "user Session expired or not logged in"
                }, {
                    status: 400
                })
        }
    ...
    return NextResponse.json({ message: "Messages fetched successfully", messages, messageCount }, { status: 200 });
