    ...
    return (
        <div className="flex items-center justify-between border-b border-gray-200 p-4">
            ...
            <button
                ...
                disabled={loading}
                className={`px-4 py-2 cursor-pointer rounded-md bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition duration-300 ${loading ? 'cursor-not-allowed' : ''} ${error ? 'bg-red-600' : ''}` }
            >
                {(loading && !error) ? "Assigning..." :(error) ? `${error}` : "Assign"}
            </button>
        </div>
    );
